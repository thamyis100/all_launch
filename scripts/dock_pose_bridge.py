...
class DockMetrics(BaseMetricsNode):
    ...
    def __init__(self):
        ...
        self.declare_parameter("max_detected_dock_pose_age", 1.0)
        ...
        docking_status_topic = self.get_parameter("docking_status_topic").get_parameter_value().string_value
        docking_status_marker_topic = self.get_parameter(
            "docking_status_marker_topic"
        ).get_parameter_value().string_value
        detected_dock_pose_source_topic = self.get_parameter(
            "detected_dock_pose_source_topic"
        ).get_parameter_value().string_value
        detected_dock_pose_topic = self.get_parameter(
        ...
        self.latest_detected_dock_pose = None
        self.last_docking_phase = "IDLE"
        self.create_subscription(
            String,
            detected_dock_pose_source_topic,
            self.on_detected_dock_pose_source,
            10,
        )
        self.create_subscription(
            PoseStamped,
            detected_dock_pose_topic,
            self.on_detected_dock_pose,
            10,
        )
    ...
    def on_detected_dock_pose_source(self, msg: String) -> None:
        ...
        if self.metrics.active and self.metrics.run_mode in ("DOCK", "UNDOCK"):
            self.publish_docking_status(
                f"{self.metrics.run_mode}: {self.last_docking_phase}",
    ...
    def _is_detected_pose_fresh(self, pose: PoseStamped) -> bool:
        if pose is None:
            return False
    ...
    def handle_docking_result(self, future, action_name: str) -> None:
        ...
        super()._on_action_result(future)
    ...
    def on_dock_pose(self, msg: PoseStamped):
        ...
        if (
            ...
            self.get_logger().error(
                "Refusing dock command: detected source is fallback_rejected_tag_tf. "
                "This is usually an unsafe pose mismatch. Update /dock_pose near the real dock "
                "or relax apriltag consistency checks."
            )
            self.publish_docking_status(
        ...
        if (
            ...
            self.get_logger().info(
                "Using fresh /detected_dock_pose for dock goal (source=apriltag_tf)."
            )

        goal_x = goal_pose_msg.pose.position.x
        goal_y = goal_pose_msg.pose.position.y
        ...
        self.get_logger().info(
            ...
            f"frame={goal_pose_msg.header.frame_id or self.global_frame}"
        )
        self.last_docking_phase = "REQUESTED"
        self.publish_docking_status(
            "DOCK: REQUESTED",
    ...
    def _on_dock_goal_response(self, future):
        ...
        if not goal_handle.accepted:
            ...
            self.publish_docking_status(
                "DOCK: REJECTED",
                "Action server rejected the request",
                1.0,
                0.2,
                0.2,
            )
    ...
    def on_undock_trigger(self, msg: Empty):
        ...
        if self.metrics.active:
            if self.metrics.run_mode == "DOCK" and self._dock_goal_handle is not None:
                self.get_logger().warn("Undock pressed during DOCK. Canceling active dock goal.")
                self.last_docking_phase = "CANCEL_REQUESTED"
                self.publish_docking_status(
                    "DOCK: CANCEL REQUESTED",
                    "Stopping docking and returning to IDLE",
                    1.0,
                    0.7,
                    0.2,
                )
        ...
        if not self._undock_ac.wait_for_server(timeout_sec=5.0):
            ...
            self.metrics.active = False
            self.publish_docking_status(
                "UNDOCK: SERVER MISSING",
                "/undock_robot unavailable",
                1.0,
                0.2,
                0.2,
            )
    ...
    def _on_undock_action_result(self, future):
        self._undock_goal_handle = None
        self.handle_docking_result(future, "UNDOCK")

    def _on_dock_cancel_response(self, future):
        try:
        ...
        if canceled:
            self.get_logger().info("Dock goal cancel accepted.")
            self.publish_docking_status(
                "Docking: IDLE",
                "Canceled by Undock button",
                0.8,
                0.8,
                0.8,
            )
        else:
            self.get_logger().warn("Dock cancel was not accepted by action server.")
            self.publish_docking_status(
                "DOCK: CANCEL FAILED",
                "Action server did not cancel goal",
                1.0,
                0.3,
                0.3,
            )
